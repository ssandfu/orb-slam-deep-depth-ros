...
public:
    ...
    // Input images: RGB (CV_8UC3) or grayscale (CV_8U). RGB is converted to grayscale.
    // Returns the camera pose (empty if tracking fails).
    void TrackMonocularDepth(const cv::Mat &im, const double &timestamp);
    // Process the given stereo frame. Images must be synchronized and rectified.
    // Input images: RGB (CV_8UC3) or grayscale (CV_8U). RGB is converted to grayscale.
    // Returns the camera pose (empty if tracking fails).
    void TrackMonocularDepth_Stereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp);

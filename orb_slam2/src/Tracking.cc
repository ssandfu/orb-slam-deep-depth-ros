    ...
    is_Frame_Tracked(false)
{
    ...
    //MonoDepth ONLY for declaration (will be set in the System correctly):
    nKeyFrameEnhance = parameters.nKFEnhance;
    std::cout << "Tracking nKFE initialized to: " << nKeyFrameEnhance << " From " << parameters.nKFEnhance << std::endl;
    kfCntr = 0;
    std::cout << "kfCntr initialized." << std::endl;
...
cv::Mat Tracking::GrabImageMonocularDepth_Stereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp)
{
    mImGray = imRectLeft;
    cv::Mat imGrayRight = imRectRight;
    ...
    mCurrentFrame_Monocular = Frame(mImGray,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
     ...
     //From Stereo. It can be assumed that the benefits of initialising with depth information is preferable to Mono initialising
    //mCurrentFrame = Frame(mImGray,imGrayRight,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);

    //nKeyFrameEnhance == 0 means that this feature is deactivated (Stereo will always be tracked). 
    //The monoDepth decision has to happen before i.e. call either this function or "GrabImageMonocularDepth"
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET || this->nKeyFrameEnhance==0)
        mCurrentFrame = mCurrentFrame_Stereo;
    else
        mCurrentFrame = mCurrentFrame_Monocular;
    ...
    else
    {
        ...
        if(bOK)
        {
            ...
            if(bnewKeyFrame){
                ...
                bool b2;
                if (useKFEnhance)  
                    b2 = (kfCntr % nKeyFrameEnhance) == 0;
                else
                    b2 = false;
                //std::cout << "KF enhancement at "<< kfCntr << ": "<< b1 << " && "<< b2 << std::endl;
                if( (b1) && (b2)){
                    //mCurrentFrame = mCurrentFrame_Stereo;
                    mCurrentFrame.mDescriptorsRight = mCurrentFrame_Stereo.mDescriptorsRight;
...
void Tracking::MonocularDepthInitialization()
{
    /*
    if(mbf <= 1.001)
